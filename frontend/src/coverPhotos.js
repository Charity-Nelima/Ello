...
export function getCoverPhotoURL(bookId) {
  const filename = coverPhotosData[bookId];
  if (!filename) {
  ...
  if (filename.startsWith('http://') || filename.startsWith('https://')) {
    return filename; // Return absolute URL
  } else {
    // Construct the relative URL to the photo in the frontend directory
    const photoURL = `/assets/${filename}`;
    return photoURL;
  }
}
